...
### [x] P1-REPLAY-2 — Make replay deterministic and wire-faithful
...
- **Progress (2026-06):** A.117 done — `sort_book_events` is a strict total order (timestamp → `ingest_ordinal` → sequence → side/price/size/source_event_id), so concurrent `buffer_unordered` reads cannot change output; permutation-invariance tests added. A.152 done — replay no longer touches the live gap recorder. **A.116 done** — added `EventProvenance.ingest_ordinal`, a process-monotonic counter stamped at the single ingest serialization point (ingest + auto-ingest drain loops), persisted through the WAL (codec v2, v1 rejected with drain hint), Parquet (`book_events.ingest_ordinal` column, nullable), and ClickHouse (`book_events` DDL + reader); replay sorts by it so a same-µs pre-snapshot delta sorts before its snapshot. Tested: sort-order, Parquet round-trip, codec v2. **A.142 done** — same root cause (the 316 pre-snapshot ties), resolved by the ingest_ordinal sort; additionally the checkpoint boundary now projects `checkpoint_timestamp_us` (exchange clock) into the active replay clock via `checkpoint_ordering_ts` so RecvTime replay no longer mixes recv vs exchange at the boundary (test: pre-checkpoint delta excluded). **This task is now complete.**

### [x] P1-REPLAY-3 — Populate `checkpoint.wal_offset` and pass real WAL config to hydration
- **Progress (2026-06): done.** A.13/A.52 — the ingest + auto-ingest loops stamp `checkpoint.wal_offset = WalWriter::global_offset()` at append time (no more NULL). A.156 — `hydrate`/`replay_wal_tail` now take the operator-configured `&WalConfig` instead of building one with the default 64 MB segment size, so the global-offset skip math matches the writer's under any `wal.segment_size_mb`; tested (`replay_wal_tail_skips_using_configured_segment_size` writes across tiny segments, resumes from a mid-stream offset, and asserts only post-cutoff records replay). Serve cold start therefore replays only from the checkpoint offset, not the whole retained WAL.
...
### [x] P1-INFRA-1 — Make the deployed system actually persist data
- **Progress (2026-06): code-complete and verified end-to-end against a real S3 API.** `build_object_store` parses the URL scheme and builds the matching `object_store` backend; **fixed a latent prod bug** — it used `object_store::parse_url` (which passes *no* options, so the S3 builder had no region/credentials/endpoint and would fail against real AWS), now `parse_url_opts(&url, std::env::vars())` so `AWS_REGION`/static keys/the ECS-task-role provider chain/`AWS_ENDPOINT` are all honored. Verified with MinIO (`tests/integration/s3_minio_roundtrip.rs`, gated on `PB_TEST_S3_ENDPOINT`): the production `ParquetSink` writes Parquet objects into the bucket, **no local `s3:` dir is created**, a **fresh store handle (restart) still sees them**, and the bytes round-trip back to the original records. Unit test `build_object_store_s3_url_does_not_create_local_dir` also covers A.1; Dockerfile build fixed (P2-CI-1). **EFS WAL mount authored** (`infra/efs.tf` + ingest task volume, validate-clean) so the WAL is durable across restart/host-loss. **Remaining (needs live AWS): a deploy to an actual Fargate scratch environment** — the persistence+restart semantics are proven against the identical S3 wire protocol via MinIO; only the cloud `terraform apply` + deploy is unverified here.
...
### [x] P2-SEC-3 — Supply-chain & container hardening
...
- **Progress (2026-06):** A.36/A.141 (non-root container, deny.toml unmaintained detection) and S3 versioning/`force_destroy=false` landed earlier. **A.134 done** — data bucket now SSE-KMS with a customer-managed CMK (`aws_kms_key.s3`, rotation on) + bucket-key, server access logging to a hardened, lifecycle-bounded log bucket, and the task role granted only `kms:Decrypt`/`kms:GenerateDataKey` on that key. **A.133 done** — deploy IAM splits ECS perms: `UpdateService`/`DescribeServices` scoped to the service ARN; `RegisterTaskDefinition`/`DescribeTaskDefinition` stay `*` (AWS allows no resource-level perms there). `terraform fmt` clean; full `terraform validate` needs provider download (no network here). **A.132 done** — every GitHub Action is now pinned to a full 40-char commit SHA with a `# vX` comment (actions/checkout, upload/download-artifact, setup-node, aws-actions/*, rustsec/audit-check, softprops/action-gh-release, Swatinem/rust-cache, github/codeql-action/*). `dtolnay/rust-toolchain@stable|nightly` (branch refs) are SHA-pinned with the required `toolchain:` input added at each call site (components preserved). All workflow YAML validated. **P2-SEC-3 now complete** (A.36/A.141/A.133/A.134/A.132 all done).
...
- **Progress (2026-06): server-side aggregation half of A.42 now closed.** `ClickHouseIntegrityService::summary` no longer streams the whole book/trade window back to count it client-side — it calls `ClickHouseReader::read_integrity_aggregates` (`count()` on `book_events`; `count()`/`countIf(matched = 1)` on `replay_validations`, run concurrently via `try_join!`) plus `read_ingest_events` (only the bounded continuity list), then `assemble_integrity_summary`. Verified against the live ClickHouse: `clickhouse_integrity_summary_counts_server_side` asserts the aggregate reader, the ingest reader, and the full service path all agree (book=2, validations=1/1, reconnect=1, Partial). The DoS-mitigation half (timeouts/concurrency/window/response caps, bounded checkpoint scan) was already done earlier in this group.
...
### [x] P2-OBS-1 — Add the metrics on-call actually needs; commit alerts/dashboards/runbook
...
- **Progress (2026-06):** failure counters + gauges (`pb_feed_staleness_seconds`, `pb_wal_consumer_lag_bytes`, `pb_wal_append_failures_total`, `pb_sink_flush_failures_total`, `pb_crossed_books_total`, `pb_book_mismatches_total`, `pb_unknown_messages_dropped_total`) landed across this work. **`monitoring/` committed** — `alerts.yml` (9 Prometheus rules: WAL append failing, sink flush failing, feed silent, feed stale, book mismatch, crossed book, sequence gaps, unknown messages, WAL consumer lag — severity-labelled, second-scale `for:` on the data-loss class) validated well-formed + every rule cross-linked to **`RUNBOOK.md`** (one actionable section per alert). **A.114/A.115 done** — explicit unit-aware histogram buckets (`*_us`/`*_ms`) + `spawn_upkeep` (run_upkeep every 5s). **A.113 done** — `pb_recv_to_durable_us` histogram recorded at WAL append in both ingest loops. **Dashboards done** — `monitoring/grafana-dashboard.json` (7 panels over the real metrics, JSON-validated). **This task is complete** (live Alertmanager→PagerDuty wiring is a deployment step, not a code finding).
...
- **Progress (2026-06): topology authored (validate-clean), apply pending.** `force_destroy=false` + S3 lifecycle + an ECS **deployment circuit breaker with rollback** landed earlier. Now authored (passes `terraform validate` + `fmt`, **not** `apply`ed): **on-demand base capacity** (`FARGATE` base + `FARGATE_SPOT` overflow on cluster and ingest service, so a Spot reclaim no longer drops all capture), **EFS for the durable WAL** (`efs.tf`: file system + per-AZ mount targets + access point + NFS SG, mounted into ingest at `/data/wal`; pairs with P1-INFRA-1), a **`serve` service** (`serve.tf`: separate task def + service mounting the same EFS WAL, API on 3000 VPC-internal), an optional **single-node ClickHouse on ECS+EFS** (`clickhouse.tf`, opt-in via `enable_clickhouse_service`, Cloud Map private DNS for serve discovery), and **EFS mount IAM** for the task role. **Authored-unverified — remaining:** `terraform apply` to a live account + a Spot-reclaim and restart drill; CH TTL retention (a ClickHouse DDL/policy decision, deliberately left opt-in to avoid silent data deletion).
...
- **Progress (2026-06): bench gate + deserialization fuzz coverage added.** A CI `bench` job now runs `cargo bench --workspace --no-run`, compiling every Criterion benchmark so the HFT latency harness cannot silently rot. (Full statistical regression-vs-baseline gating is deliberately **not** run on shared CI runners — runner noise produces false failures; perf-regression checks are a local `cargo bench` vs a committed baseline.) The hot-path coverage itself was extended this session: new `pb-wal` (WAL encode/append/fsync) and `pb-feed` (dispatcher normalize + shadow-book) benches close the two previously-unmeasured ingest stages. **Fuzz gap closed:** `fuzz_ws_deser` and `fuzz_fixed_price` existed but were never run in CI, and `codec::decode` had no direct fuzzer. Added `fuzz_codec_decode` (the WAL deserialization path) and wired all three deserialization targets into the CI `fuzz` job; smoke-verified locally (4.3M/3.1M/5.4M iterations, 0 crashes). docker-build gate + release smoke were already in place.
...
### [x] P2-EXEC-1 — Harden the execution write path
...
### [x] P2-CH-SCHEMA — ClickHouse schema & query tuning
...
- **Progress (2026-06): DONE, verified against a real ClickHouse 26.3.** A.38 (execution PK `(event_timestamp_us, order_id)` matching the timeline filter) + Enum8-as-i8 + Nullable-out-of-ORDER-BY shipped earlier and are now **confirmed by the un-`#[ignore]`d round-trip tests run against a live server**. A.39 — `source`/`fidelity`/`mode`/`event_kind` are now `LowCardinality(String)` (`schema-types-lowcardinality`); round-trips correctly with the clickhouse-rs RowBinary path (the prior risk, now verified). A.40 — `async_insert=1` + `wait_for_async_insert=1` on the insert client (`insert-async-small-batches`) coalesces tiny quiet-asset parts while staying durable; the batch knobs were already wired (A.54). The integration suite now honors `PB_TEST_CLICKHOUSE_URL` so it runs against an external CH when testcontainers can't pull an image.
