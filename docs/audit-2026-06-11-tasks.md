...
### [ ] P1-STORE-3 — Close the Parquet data-loss and silent-overwrite windows
...
- **Progress (2026-06):** A.61 (timestamp range validation), A.62 (LatencyTrace monotonicity), A.63 (deterministic timeline tie-break), A.65 (server-side offset/order pagination across service+HTTP+gRPC+web), and A.144-partial (reject empty order_id + fill-without-price/size) are done. **Remaining: A.60** (idempotency/dedup token + ReplacingMergeTree, multi-table atomicity) and **A.124/A.144** (stateful order-lifecycle validation, e.g. fill-after-cancel).
