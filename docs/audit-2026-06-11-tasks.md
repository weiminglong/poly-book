...
- **Progress (2026-06):** A.111 done — `RestClient` connect (5s) + request (15s) timeouts. A.110 done — undeserializable frames increment `pb_unknown_messages_dropped_total`. **A.74/A.109 done** — the dispatcher now maintains a per-asset shadow `L2Book`, applies each price-change delta to it, and cross-checks our top-of-book against the venue-stated `best_bid`/`best_ask` on every entry; on divergence it emits an `IngestEventKind::BookMismatch` ingest event (a queryable data hole) + `pb_book_mismatches_total`. Shadow books reset on continuity reset. Tests: `top_price_mismatch` cases, end-to-end mismatch-emitted and no-false-positive. **Resnapshot trigger (dispatcher side) done** — the dispatcher now emits a resnapshot request for the diverging asset on an optional `resnapshot_tx` channel (`with_resnapshot_tx`; non-blocking try_send so detection never backpressures ingest); tested. **Remaining:** the consumer worker that receives requests, fetches a REST snapshot, and re-injects it as a synthetic WS `book` message into `raw_tx` (reusing all existing snapshot handling) — left for wiring/verification against a real feed since the re-injected JSON must match the live wire format exactly. The venue opaque `hash` is not recomputable, so best_bid/ask is the validation signal.
