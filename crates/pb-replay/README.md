...
| `ReplayResult` | Output of reconstruction: the rebuilt book, event count, checkpoint info, validation status. |
...
Parquet files / ClickHouse tables
  ...
  ReplayEngine::reconstruct_at(asset_id, target_us)
        │
        ▼
  ReplayResult { book, events_applied, checkpoint_used, ... }
        │
        ▼
...
- The `EventReader` trait has methods for reading each dataset type:
  `read_market_data`, `read_checkpoints`, `read_validations`,
